# yaGPT to SQL

Программа для генерации SQL запросов к ClickHouse с использованием Yandex GPT.

## Описание

Эта программа принимает запросы пользователя на естественном языке и генерирует SQL запросы для ClickHouse, используя Yandex GPT API. Программа автоматически получает информацию о структуре таблицы и передает её в контекст модели для более точной генерации запросов.

**📖 Важно:** Реализованы рекомендации Yandex WebSQL Concierge для корректной генерации SQL. См. [YANDEX_ASSISTANT_ANALYSIS.md](YANDEX_ASSISTANT_ANALYSIS.md) для деталей.

## Возможности

- 🤖 Генерация SQL запросов с использованием Yandex GPT (полная модель рекомендуется)
- 📊 Автоматическое получение структуры таблицы из ClickHouse
- 🔍 Контекстная генерация с учётом типов данных и описаний столбцов
- 🎯 Структурированный JSON-контекст для точной генерации
- ⚡ Поддержка работы с данными из Яндекс Метрики
- 🛠️ Возможность выполнения сгенерированных запросов
- 🔒 Поддержка SSL сертификатов для облачных сервисов

## Требования

- Python 3.7+
- Доступ к Yandex Cloud API (API ключ и folder ID)
- Доступ к ClickHouse базе данных
- **Рекомендуется:** Использование полной модели `yandexgpt` (не lite) для лучших результатов

## Установка

1. Клонируйте репозиторий:
```bash
git clone https://github.com/Erofaxxx/yaGPT_to_SQL.git
cd yaGPT_to_SQL
```

2. Установите зависимости:
```bash
pip install -r requirements.txt
```

3. **ВАЖНО:** Создайте файл `.env` на основе `.env.example`:
```bash
cp .env.example .env
```

4. **Обязательно** заполните файл `.env` вашими данными:
```bash
# Yandex Cloud API
YANDEX_API_KEY=ваш_api_ключ
YANDEX_FOLDER_ID=ваш_folder_id

# Рекомендуется: используйте полную модель для лучших результатов
YANDEX_GPT_MODEL=yandexgpt  # Или yandexgpt-lite для экономии

# ClickHouse
CLICKHOUSE_HOST=адрес_clickhouse
CLICKHOUSE_PORT=8123
CLICKHOUSE_USER=пользователь
CLICKHOUSE_PASSWORD=пароль
CLICKHOUSE_DATABASE=база_данных
CLICKHOUSE_TABLE=название_таблицы

# SSL сертификат (для облачных сервисов)
CLICKHOUSE_SSL_CERT_PATH=YandexInternalRootCA.crt
```

**Примечание:** Программа автоматически загружает переменные окружения из файла `.env` в текущей директории. Убедитесь, что файл `.env` создан и правильно заполнен перед запуском программы.

## Использование

### Запуск программы

```bash
python yagpt_to_sql.py
```

Программа запросит ваш запрос на естественном языке или использует пример по умолчанию.

### Примеры запросов

1. **Топ пользователей**:
   ```
   Напиши запрос, который найдёт топ-5 самых активных пользователей по количеству событий за последние 7 дней, с группировкой по стране.
   ```

2. **Анализ сессий**:
   ```
   Составь запрос, который агрегирует данные по сессиям пользователей за вчера, с фильтрацией по мобильным устройствам и сортировкой по длительности.
   ```

3. **Статистика по источникам**:
   ```
   Выведи статистику по источникам трафика за последний месяц с группировкой по дням.
   ```

### Использование как библиотеки

```python
from yagpt_to_sql import YandexGPTSQLGenerator, ClickHouseHelper

# Инициализация генератора SQL
sql_generator = YandexGPTSQLGenerator(
    api_key="ваш_api_ключ",
    folder_id="ваш_folder_id"
)

# Инициализация помощника ClickHouse
ch_helper = ClickHouseHelper(
    host="localhost",
    port=8123,
    user="default",
    password="",
    database="default"
)

# Получение информации о таблице
table_info = ch_helper.get_table_info("metrika_hits")

# Генерация SQL запроса
user_request = "Найди топ-10 страниц по количеству просмотров за сегодня"
sql_query = sql_generator.generate_sql(user_request, table_info)

print(sql_query)

# Выполнение запроса (опционально)
results = ch_helper.execute_query(sql_query)
```

## Настройка

### Переменные окружения

- `YANDEX_API_KEY` - API ключ Yandex Cloud
- `YANDEX_FOLDER_ID` - ID каталога в Yandex Cloud
- `CLICKHOUSE_HOST` - Адрес сервера ClickHouse
- `CLICKHOUSE_PORT` - Порт ClickHouse (по умолчанию 8123)
- `CLICKHOUSE_USER` - Имя пользователя ClickHouse
- `CLICKHOUSE_PASSWORD` - Пароль пользователя ClickHouse
- `CLICKHOUSE_DATABASE` - Имя базы данных
- `CLICKHOUSE_TABLE` - Имя таблицы с данными Яндекс Метрики

### Параметры генерации

В методе `generate_sql()` можно настроить:
- `temperature` (0.0-1.0) - контролирует случайность генерации (по умолчанию 0.3)
- `max_tokens` - максимальное количество токенов в ответе (по умолчанию 500)

## Архитектура

Программа состоит из двух основных классов:

### YandexGPTSQLGenerator

Отвечает за взаимодействие с Yandex GPT API:
- Формирует контекст с информацией о таблице
- Отправляет запросы к API
- Обрабатывает ответы и извлекает SQL

### ClickHouseHelper

Отвечает за работу с ClickHouse:
- Получает структуру таблицы (столбцы, типы, описания)
- Выполняет SQL запросы
- Возвращает результаты в удобном формате

## Документация

- [ClickHouse Python интеграция](https://clickhouse.com/docs/integrations/python)
- [Yandex GPT API](https://cloud.yandex.ru/docs/yandexgpt/)
- [Yandex Метрика](https://metrika.yandex.ru/)

## Особенности

- Программа учитывает, что SQL запросы будут использоваться для выгрузки срезов данных с последующей обработкой в Python
- Не обязательно полностью вычислять всё в SQL - некоторые анализы проще делать в Python
- Генератор оптимизирован для работы с данными Яндекс Метрики

## Устранение неполадок

### Программа использует значения по умолчанию вместо моих настроек

**Проблема:** При запуске программа показывает `База данных: default` и `Таблица: metrika_hits` вместо значений из конфигурации.

**Решение:**
1. Убедитесь, что файл `.env` создан в директории проекта (используйте `ls -la`, чтобы увидеть скрытые файлы)
2. Убедитесь, что запускаете программу из директории, где находится файл `.env`
3. Проверьте, что в файле `.env` нет опечаток в именах переменных
4. Установите `python-dotenv` если еще не установлен: `pip install python-dotenv`

Для проверки загрузки переменных окружения выполните:
```bash
python test_env.py
```

### Ошибки подключения к ClickHouse (Найдено столбцов: 0)

**Проблема:** Программа загружает правильную конфигурацию, но не может найти таблицу.

**Диагностика:** Программа теперь выводит подробные логи ошибок. Запустите:
```bash
python yagpt_to_sql.py
```

Вы увидите детальные сообщения об ошибках, например:
- `Ошибка соединения с ClickHouse: Connection refused` - ClickHouse недоступен
- `Ошибка выполнения запроса: 401` - Неверные учетные данные
- `Ошибка выполнения запроса: 404` - База данных или таблица не найдены
- `SSL: CERTIFICATE_VERIFY_FAILED` - Проблема с SSL сертификатом

**Решение:**

1. **Проверьте настройки подключения:**
   ```bash
   python test_connection.py
   ```

2. **Убедитесь, что хост настроен правильно:**
   
   Программа поддерживает два формата указания хоста:
   
   **С указанием протокола (рекомендуется для облачных сервисов):**
   ```
   # Yandex Managed ClickHouse (HTTPS + порт 8443)
   CLICKHOUSE_HOST=https://your-cluster.mdb.yandexcloud.net
   CLICKHOUSE_PORT=8443
   ```
   
   **Без указания протокола (для локальных серверов):**
   ```
   # Локальный ClickHouse (HTTP + порт 8123)
   CLICKHOUSE_HOST=localhost
   CLICKHOUSE_PORT=8123
   ```
   
   **Важно:** Для защищенных подключений (HTTPS) обязательно укажите `https://` в начале хоста.

3. **Проверьте права доступа:**
   - Таблица существует в указанной базе данных
   - У пользователя есть права на чтение system.columns
   - Учетные данные верны

4. **Включите отладочные логи:**
   Добавьте в `.env`:
   ```
   LOG_LEVEL=DEBUG
   ```
   И запустите программу снова для более подробной информации.

### Ошибка SSL сертификата (CERTIFICATE_VERIFY_FAILED)

**Проблема:** При подключении к Yandex Managed ClickHouse или другим облачным сервисам появляется ошибка:
```
SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed: self signed certificate in certificate chain
```

**Причина:** Облачные сервисы могут использовать самоподписанные сертификаты или сертификаты, которые не могут быть проверены стандартным образом.

**Решение (рекомендуемое):**

1. Используйте предоставленный файл сертификата. Добавьте в `.env`:
   ```
   CLICKHOUSE_SSL_CERT_PATH=YandexInternalRootCA.crt
   ```

2. Запустите программу:
   ```bash
   python yagpt_to_sql.py
   ```

Программа будет использовать указанный сертификат для проверки SSL соединения. Это безопасно и рекомендуется для продакшена.

**Альтернативное решение (временное, только для разработки):**

Если у вас нет файла сертификата, можно временно отключить проверку SSL:
```
CLICKHOUSE_SSL_VERIFY=false
```

**⚠️ ВНИМАНИЕ:** Отключение проверки SSL сертификатов делает соединение уязвимым для атак типа "человек посередине" (MITM). Используйте это только для разработки и тестирования!

**Варианты конфигурации:**

1. **С файлом сертификата** (рекомендуется):
   ```
   CLICKHOUSE_SSL_CERT_PATH=YandexInternalRootCA.crt
   ```

2. **Системные сертификаты** (по умолчанию):
   ```
   # Не указывайте CLICKHOUSE_SSL_CERT_PATH
   # Будут использованы системные сертификаты
   ```

3. **Отключить SSL проверку** (небезопасно):
   ```
   CLICKHOUSE_SSL_VERIFY=false
   ```

## Лицензия

MIT

## Автор

Erofaxxx
